...
static char *
ngx_http_upstream_dynamic_server_directive(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    for (i = 2; i < cf->args->nelts; i++) {
        ...
        if (ngx_strcmp(value[i].data, "resolve") == 0) {
            ...
            if (!u.addrs || !u.addrs[0].sockaddr) {
                ...
                dynamic_server->pool = ngx_create_pool(NGX_DEFAULT_POOL_SIZE, ngx_cycle->log);
                ...
                dynamic_server->spread = spread;
            }

            continue;
        }

        // prevent dns query denied    
        if (ngx_strncmp(value[i].data, "spread=", 7) == 0) {
            s.len = value[i].len - 7;
            s.data = &value[i].data[7];

            spread = ngx_parse_time(&s, 0);

            if (spread == (time_t) NGX_ERROR) {
...
ngx_http_upstream_dynamic_servers_init_process(ngx_cycle_t *cycle) {
    ...
    for (i = 0; i < udsmcf->dynamic_servers.nelts; i++) {
        ...
        //refresh_in = ngx_random() % 10000;
        refresh_in = ngx_random() % dynamic_server[i].spread;
...
reinit_upstream:
    ...
    ngx_reset_pool(dynamic_server->pool);

    dynamic_server->server->addrs = ngx_pcalloc(dynamic_server->pool, ctx->naddrs * sizeof(ngx_addr_t));
    if (dynamic_server->server->addrs == NULL) {
        ngx_log_error(NGX_LOG_ERR, ctx->resolver->log, 0, "upstream-dynamic-servers: Error allocated memory addrs");
        goto end;
    }
    for (i = 0; i < ctx->naddrs; i++) {
        ngx_memcpy(&dynamic_server->server->addrs[i], &ctx->addrs[i], sizeof(ngx_addr_t));
    }
    ...
    for (i = 0; i < ctx->naddrs; i++) {
        ...
        sockaddr = ngx_pcalloc(dynamic_server->pool, socklen);
        if (sockaddr == NULL) {
            ngx_log_error(NGX_LOG_ERR, ctx->resolver->log, 0, "upstream-dynamic-servers: Error allocated memory sockaddr");
            goto end;
        }
        ...
        size_t len;
        u_char *p = addr->name.data;
        if (p == NULL) {
            p = ngx_pnalloc(dynamic_server->pool, NGX_SOCKADDR_STRLEN);
            if (p == NULL) {
        ...
        len = ngx_sock_ntop(addr->sockaddr, socklen, p, NGX_SOCKADDR_STRLEN, 1);
    ...
    if (ctx->state) {
        ...
        rec = udsmcf->resolver->connections.elts;

        rec = &rec[dynamic_server->try_connection];
        ngx_log_error(NGX_LOG_DEBUG, ctx->resolver->log, 0, "upstream-dynamic-servers: '%V' could not be resolved %V (%i: %s)", &ctx->name, &rec->server, ctx->state, ngx_resolver_strerror(ctx->state));
        dynamic_server->fail_cnt++;

    } else {

        dynamic_server->fail_cnt = 0;

        hash = ngx_crc32_short(ctx->name.data, ctx->name.len);
        rn = ngx_resolver_lookup_name(ctx->resolver, &ctx->name, hash);
        if (rn != NULL) {
            dynamic_server->try_connection = rn->last_connection;
        }
    }

    if (dynamic_server->server->down == 1 && dynamic_server->fail_cnt == (udsmcf->resolver->connections.nelts)) {
        ngx_log_error(NGX_LOG_ERR, ctx->resolver->log, 0, "upstream-dynamic-servers: '%V' could not be all dns server resolved (%i: %s)", &ctx->name, ctx->state, ngx_resolver_strerror(ctx->state));
        dynamic_server->fail_cnt=0;
    }

    dynamic_server->try_connection++;
    if (dynamic_server->try_connection == udsmcf->resolver->connections.nelts) {
        dynamic_server->try_connection = 0;
    }
...
ngx_http_upstream_dynamic_server_upstream_init(ngx_conf_t *cf, ngx_http_upstream_srv_conf_t *us) {
    ...
    udsscf = ngx_http_conf_upstream_srv_conf(us, ngx_http_upstream_dynamic_servers_module);
        ...
        if (peers) {
                ngx_pfree(cf->pool, peers->peer);
                if (peers->next) {
                        ngx_pfree(cf->pool, peers->next->peer);
                        ngx_pfree(cf->pool, peers->next);
                }
                ngx_pfree(cf->pool, peers);
                ngx_pfree(cf->pool, us->peer.data);
        }

    if (udsscf->original_init_upstream(cf, us) != NGX_OK) {
        ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0, "%s, init upstream Error", __func__);
        return NGX_ERROR;
    }
...
ngx_http_upstream_dynamic_server_init_peer(ngx_http_request_t *r, ngx_http_upstream_srv_conf_t *us) {
    ...
    if (ahpd == NULL) {
                ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0, "%s, upstream-dynamic-servers: Error allocated memory", __func__);
        return NGX_ERROR;
    }

        udsscf->ahpd = ahpd;
    if (udsscf->original_init_peer(r, us) != NGX_OK) {
                ngx_pfree(r->pool, ahpd);
        return NGX_ERROR;
    }
